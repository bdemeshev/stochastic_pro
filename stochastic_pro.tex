...
\begin{problem}
  Consider a symmetric random walk $S_n$ with $S_0 = 0$. 
  One may write $S_n = X_1 + X_2 + \dots + X_n$ where the steps $X_n$ are independent and take values $+1$ or $-1$ with equal probabilities.

  Consider the first moment of time $\tau$ when $S_n$ will hit or intersect the line $n - 100$.

  \begin{enumerate}
    \item Find $\E(\tau)$ and $\Var(\tau)$.
    \item Find the probability $\P(\tau = k)$.
    \item Find the moment generating function $\mgf(u)$ of $\tau$.
    \item Solve the point (a), (b) and (c) for the line $2n - 100$.
    \item Solve the point (a) for the line $1/2 n - 100$.
  \end{enumerate}


  \begin{sol}
    \begin{enumerate}
      \item Consider the distance between the point and the line, $d_t$, and observe that $\Delta d_t$ is equal to $0$ or $1$ with equal probabilities.
    \end{enumerate}
...
\subsection*{First step into fractal}


\begin{problem}
  The Cantor set fractal $C$ is constructed in the following way. 
  We start with unit segment $I_0 = [0; 1]$.
  At the first stage we remove the middle third from the segment.
  Hence after the first stage we obtain $I_1 = [0; 1/3] \cup [2/3; 1]$.
  At the second stage we remove the middle third from every segment.
  Hence after the second stage we obtain $I_2 = [0; 1/9] \cup [2/9; 3/9] \cup [6/9; 7/9] \cup [8/9; 9/9]$.
  We continue this process infinitely many times and in the limit we obtain the Cantor set $C$,
  see \url{https://en.wikipedia.org/wiki/Cantor_set}.

  \begin{enumerate}
    \item Find the length of the Cantor set $C$.
  \end{enumerate}
  Now let's choose a point $X$ in the Cantor set randomly uniformly. 
  \begin{enumerate}[resume]
    \item Find $\E(X)$ and $\Var(X)$.
    \item How one may randomly choose a point in the Cantor set using a fair coin?
    \item Find at least one point in the Cantor set that does is not the endpoint of any segment. 
    \item Let $F$ the cumulative distribution function of $X$. Find the values $F(1/2)$, $F(1/3)$, $F(1/4)$ and $F(1/5)$.
  \end{enumerate}

  \begin{sol}
    \begin{enumerate}
      \item $1 - 1/3 - 2/9 - 4/27 - \dots = 1 - \frac{1/3}{1 - 2/3} = 0$;
      \item $\E(X) = 1/2$ by symmetry;
      For the $\E(X^2)$ we may build an equation using the first step approach
      \[
      \E(X^2) = 0.5 \E( (X/3)^2) + 0.5 \E((2/3 + X/3)^2).
      \]
      From this equation one obtains $\E(X^2) = 3/8$ and hence $\Var(X) = 1/8$.
      
      Or one may also use the formula for the variance 
      \[
      \Var(X) = \E(\Var(X \mid Y)) + \Var(\E(X \mid Y)).
      \]
      Here $Y = I(X > 1/2)$ indicates whether $X$ belongs to the left or right side of the fractal. 
      \[
      \Var(X \mid Y) = \Var(X) / 9
      \]
      Observe that $\E(X \mid Y)$ takes only two values, $1/6$ or $5/6$. 
      And hence $\Var(\E(X \mid Y)) = 1/9$.
      \[
      \Var(X) = \Var(X) / 9 + 1/9
      \]
      And, finally $\Var(X) = 1/8$.

      \item Let's toss the coin infinitely many times. 
If we observe $H$ then we choose the left part and if we observe $T$ then we choose the right part during the stage. 
      \item Let's go left-right-left-right-\dots 
      \item $F(1/2) = 1/2$, $F(1/3) = 1/2$, $F(1/4) = $, $F(1/5) = $
    \end{enumerate}
...
\begin{problem}
  The Sierpiński triangle $S$ is constructed in the following way. 
  We start with an equilateral triangle $T_0$ with unit sides. 
  At the first stage we subdivide the triangle into four smaller congruent equilateral triangles and remove the central triangle.
  Hence after the first stage the set $T_1$ consists of three triangles.
  At the second stage we we subdivide every triangle into four smaller congruent equilateral triangles and remove the central triangle.
  Hence after the second stage we obtain the set $T_2$ that consists of nine triangles.
  We continue this process infinitely many times and in the limit we obtain the Sierpiński triangle $S$,
  see \url{https://en.wikipedia.org/wiki/Sierpiński_triangle}.

  \begin{enumerate}
    \item Find the are of the Sierpiński triangle $S$.
  \end{enumerate}
  Now let's choose a point $(X, Y)$ in the Sierpiński triangle with vertices $(0,0)$, $(1,0)$ and $(1/2, \sqrt{3}/2)$ randomly uniformly. 
  \begin{enumerate}[resume]
    \item Find $\E(X)$ and $\Var(X)$.
    \item Find $\E(Y)$ and $\Var(Y)$.
    \item Find $\Cov(X, Y)$.
    \item How one may randomly choose a point in the Sierpiński triangle using a fair coin?
  \end{enumerate}

  \begin{sol}
  \begin{enumerate}
    \item $1 - 1/4 - 3/4^2 - 3^2/4^3 - \dots = 1 - \frac{1/4}{1 - 3/4} = 0$;
    \item $\E(X) = 1/2$ by symmetry;
    For the $\E(X^2)$ we may build an equation using the first step approach
    \[
    \E(X^2) = 1/3 \E( (X/2)^2) + 1/3 \E((1/2 + X/2)^2) + 1/3\E((1/4 + X/2)^2).
    \]
    \item 
    \item $\Cov(X, Y) = 0$.
    \item Toss a fair coin infinitely many times. 
    Divide the tosses into pairs. 
    If we observe the pair $HH$ we choose the left smaller triangle, $TH$ — the right one, $HT$ — the upper one, $TT$ — discard the pair of tosses.
  \end{enumerate}
  \end{sol}
\end{problem}
...
\begin{problem}
  ...
  Your initial capital is $10000$ dollars.
...
\begin{problem}
A monkey is randomly typing letters. 
...
\begin{problem}
  Let $X_n$ be a discrete time stochastic process that converges in probability to a random number $X$ as $n\to \infty$. 
  \begin{enumerate}
...
Let $X_n$ be the indicator that the coin landed on Head at toss number $n$.
Consider a pack of $ \sigma$-algebras: $\cF_n:=\sigma(X_1, X_2, \ldots, X_n)$, $\cH_n:=\sigma(X_n, X_{n+1}, X_{n+2}, \ldots)$.
...
\item $A\in \cF_{2024}$;
\item $A\notin \cF_{2025}$;
\item $A \in \cH_n$ for all possible $n$;
\end{enumerate}
...
\begin{problem}
HSE student rolls the dice once. 
...
% wrt to random variables or sigma-algebras
...
\begin{problem}
  Random variables $X$ and $Y$ are independent with finite expected values and finite variances. 

  Elon Musk knows that $\E(X \mid X + Y) = X + Y$.
  \begin{enumerate}
    \item Find $\E(Y \mid X + Y)$ and $\E(Y)$.
    \item Find $\Var(Y)$ and $\Var(X \mid X + Y)$.
    \item What is the distribution of the random variable $Y$?
  \end{enumerate}
  \begin{sol}
    \begin{enumerate}
      \item $\E(Y \mid X + Y) = 0$ and $\E(Y) = 0$.
      \item Using the formula $\Var(X) = \Var(\E(X \mid X + Y)) + \E(\Var(X \mid X + Y))$,
      we obtain that $\Var(Y) + \E(\Var(X \mid X + Y)) = 0$.

      Hence, $\Var(Y) = 0$ and $\Var(X \mid X + Y) = 0$.
      \item The random variable $Y$ is almost surely zero.
    \end{enumerate}
  \end{sol}
\end{problem}
...
\begin{problem}
Let's sum up independent identically distributed random variables $X_t \sim \dUnif[0;1]$.
Denote the cumulative sum by $S_t = X_1 + X_2 + \dots + X_t$.

\begin{enumerate}
  \item Find $\E(S_9 \mid S_8)$, $\E(S_9 \mid S_{10})$, $\E(S_9 \mid S_8, S_{10})$, $\E(S_9 \mid S_5, S_{10}, S_{20})$.
  \item Find $\Var(S_9 \mid S_8)$.
\end{enumerate}
% TODO: is it possible to find $\Var(S_1 \mid S_{10})$?
...
\begin{problem}
At time moment $t = 0$ in the casino there are countably many players with perfect memory.
...
\begin{sol}
  \begin{enumerate}
    ...
    \item $\E(T \mid \cF_J) = T$, $\E(X_T \mid \cF_J) = 1$, $\E(X_{T+1} \mid \cF_J) = -1$.
...
\begin{problem}
  ...
  Find $\E(X\mid Y)$, $\Var(X \mid Y)$.  
  \begin{sol}
    From $\int_0^1 \int_0^1 f(x,y) \, dx\, dy=1$ we infer the value of $c$, $c=e/(e-1)^2$.

    Observe that $f(x, y)$ may be decomposed into a product, hence $X$ and $Y$ are independent, 
    hence $\E(X \mid Y) = \E(X) = 1/(e-1)$ and $\Var(X \mid Y) = \Var(X)$.
  \end{sol}
\end{problem}

\begin{problem}
  Donald Trump throws a fair coin infinitely many times.
  Let $X$ be the number of a toss when the first Tail appears.
  Let $Y$ be the result of the second toss, $1$ for Tail, $0$ for Head.
...
\begin{problem}
  Let $\Omega = \{a, b, c\}$ and the distribution of $Y$ is given by the table 
  ...
  \begin{sol}
    \begin{enumerate}
      \item Find $\E(Y \mid \cF)$, $\Var(Y \mid \cF)$, $\P(Y = 3 \mid \cF)$.
      \item $\E(Y \mid \cH) = Y$, $\Var(Y \mid \cH) = 0$, $\P(Y = 3 \mid \cH) = I_{Y=3}$.
    \end{enumerate}  
  \end{sol}
\end{problem}

\begin{problem}
Masha picked up $100$ mushrooms. 
There are three types of mushrooms, l-mushrooms, m-mushrooms and r-mushrooms. 
The types are independent and may by encountered with probabilities $0.2$, $0.3$ and $0.5$ correspondingly. 
Let's denote the total number of picked up mushrooms of each type by $L$, $M$ and $R$.
\begin{enumerate}
\item Find $\E(R+L \mid M)$, $\E(M \mid R+L)$, $\Var(R + L \mid M)$.
\item Find $\E(R \mid L)$ and $\Var(R \mid L)$.
\item Find $\E(R+L \mid L+M)$.
\item Find $\P(\E(R \mid L)=0)$ and  $\P(R=0 \mid L)$.
\item Find $\E \left( \left( \frac{0.3}{0.8} \right)^{100-L} \right)$.
\end{enumerate}

\begin{sol}
$\E(R+L \mid M)=n-M$, $\E(M \mid R+L)=n-R-L$, $\Var(R + L \mid M) = 0$, $\E(R \mid L)=(n-L)\frac{r}{1-l}$, $\Var(R \mid L)=(n-L)\frac{r}{1-l}\frac{1-r-l}{1-l}$, $\P(\E(R \mid L)=0)=l^{n}$, $\P(R=0 \mid  L)=\left(\frac{r}{r+m}\right)^{n-L}$
\end{sol}
\end{problem}
...
\begin{problem}
  At stage one I choose the random value $X$ uniformly on $[1;2]$.
  At stage two I choose the value of $Y$ randomly exponentially with rate $X$ conditionally on $X$. 

  \begin{enumerate}
    \item Find $\E(Y \mid X)$, $\Var(Y \mid X)$.
    \item Find $\E(Y)$ and $\Var(Y)$.
    \item Find $\E(X \mid Y)$ and $\Var(X \mid Y)$.
  \end{enumerate}
  \begin{sol}
    \begin{enumerate}
      \item $\E(Y \mid X)  = 1/X$, $\Var(Y \mid X) = 1/X^2$.
      \item $\E(Y) = \ln 2$, $\Var(Y) = 0.5 - (\ln 2)^2$.
      \item 
    \end{enumerate}  
  \end{sol}
\end{problem}
...
\begin{problem}
We select a point $(X, Y)$ randomly uniformly inside the square with vertices \((0,0)\), \((4,0)\), \((4,4)\), and \((0,4)\). 
\begin{enumerate}
...
Source: Stochastic Processes December 2025 exam.
  \begin{sol}
  \end{sol}
\end{problem}
...
\begin{problem}
The joint distribution of $X$ and $Y$ is given by the table:
\begin{center}
\begin{tabular}{c|ccc}
 & $Y = -1$ & $Y = 0$ & $Y = 1$ \\
\hline
...
\begin{itemize}
  ...
  \item A process $(X_n)$ is a \emph{martingale} if $\E(X_{n + k} \mid X_n, X_{n-1}, \dots, X_1) = X_n$ for all $k \geq 1$.
  ...
  \item A process $(X_n)$ is a \emph{martingale wrt to filtration} $(\cF_n)$ if $\E(X_{n + k} \mid \cF_n) = X_n$.
\end{itemize}
\end{leftbar}
...
\begin{itemize}
  ...
  \item $\P(\tau < \infty) = 1$, the process $M_t$ is uniformly integrable,
  \[
...
\begin{problem}
Jeanne Antoinette Poisson receives letters from Diderot and d'Alembert. 
...
\begin{problem}
The Legendary November Midterm Grader grades tests according to Poisson point process with variable intensity rate.
...
\begin{problem}
  Let $Y_t=W_t^3-tW_t^4$. 
  
  Find $\E(Y_t)$ and $\Var(Y_t)$.
  \begin{sol}
\[
\E(Y_t)=\E(W^3_t-tW^4_t) = \E(W^3_t)-\E(tW^4_t) = \E(W^3_t)-t \E(W^4_t)= 0 - 3t^3 = - 3t^3,
\]
To calculate $\Var(Y_t) =\E(Y^2_t)-(\E(Y))^2$ we start with
\[
...
\begin{problem}
Consider the standard Wiener process $(W_t)_{t\geq 0}$.
\begin{enumerate}
...
\begin{problem}
Consider a complex-valued stochastic process, $C_t = \exp(i W_t)$.
\begin{enumerate}
  \item Simulate three trajectories of this process and draw them on a complex plane.
\end{enumerate}
Hint: one may consider a fixed $T = 10$ and split time interval $[0; T]$ into $n = 10000$ small segments.
The Wiener process increments on these segments will be independent normal distributed random variables.

\begin{enumerate}[resume]
  \item According to you plot is $C_t$ a martingale or it has a systematic drift?
\end{enumerate}

Consider the process $M_t = h(t) \exp(i W_t)$ where $h(t)$ is a non-random function. 

\begin{enumerate}[resume]
  \item Find $dM_t$ using Ito's lemma. 
  \item Find a function $h(t)$ such that $(M_t)$ is a martingale and $M_0 = 1$.
  \item Is $\Re M_t$ a martingale?
  \item Is $\Im M_t$ a martingale?
\end{enumerate}

  \begin{sol}
    \[
    dM_t  = h(t) \exp(iW_t) i dW_t + 0.5 h(t) \exp(iW_t)i^2 (dW_t)^2 + h'(t)\exp(iW_t)dt.
    \]
    For $M_t$ to be a martingale we need $h' = 0.5 h$, $h(t) = \exp(t/2)$ and $M_t = \exp(iW_t + t/2)$.

    Both $\Re M_t = \cos(W_t) \exp(t/2)$ and $\Im M_t = \sin (W_t) \exp(t/2)$ are martingales.
  \end{sol}
\end{problem}
...
\begin{problem}
Consider the process $Q_t = \cos W_t$.
\begin{enumerate}
...
\end{enumerate}
Source: designed for Stochastic Processes December 2025 exam, but no used.
  \begin{sol}
...
\begin{problem}
Consider the process $X_t = A_t B_t$ where $A_t = \int_0^t u \, dW_u$ and $B_t = \int_0^t W_u^2 \, dW_u$.
\begin{enumerate}
...
\begin{enumerate}
\item Use Ito Lemma to find both differential and integral expressions for $Y_t$ and use them to express $X_t$ (a.s.) in terms of $X_{\infty}$, $X_0$, $\tau$, $\sigma$ and $t$. 
Here $X_0$ is the value of $X_t$ at time $t=0$.
\item Find $\E(X_t)$ and $\Var(X_t)$. 
Sketch the graph of $\E(X_t)$ as a function of $t$ for $X_{\infty}=1$, $\tau=1$, and $X_0 = 0$, $1$, and $2$. 
Plot a possible trajectory of $X_t$ in each case. 
Is there any name or names associated with $X_t$?
\end{enumerate}
  \begin{sol}
    \begin{multline*}
...
\begin{problem}
  ...
  \begin{sol}
\[
X_0 = \E_{\tilde{p}}( \exp( - rT )X_T \mid \cF_0)
\]
  \end{sol}
\end{problem}
...
\begin{problem}
  Rewrite the equation $y_t = 4 + 0.4y_{t-1} + 0.3u_{t-1} + u_t$ using lag operator.
  \begin{sol}
  \[
  (1 - 0.4L)y_t = 4 + (1 + 0.3L)u_t
  \]
  \end{sol}
  \end{problem}
  ...
  \begin{problem}
  Consider the process $(x_t)$ with $t\geq 0$ and $y_t = (1 + L)^t x_t$.
  Write $x_t$ in terms of $y_t$ using lag operator $L$.
  \begin{sol}
  $x_t=(1-L)^{t}y_t$
  \end{sol}
  \end{problem}
  ...
  \begin{enumerate}
  \item Write the recurrence relation of Fibonacci numbers using lag operator. 
  \item Simplify the expression $L^3 (1 + L)^3 F_n$.
  \item Simplify the expression $L^{13} (1 + L)^{10} F_n$.
  \item Simplify the fraction
  \[
  \frac{F_{101}+C^{1}_{5}F_{102}+C^{2}_{5}F_{103}+C^{3}_{5}F_{104}+C^{4}_{5}F_{105}+C^{5}_{5}F_{106}}
  {F_{111}}
  \]
  \item Simplify the value of $a$.
  \end{enumerate}
  
  \begin{sol}
  Recall that $F_n=L(1+L)F_n$, hence $F_n=L^k (1+L)^k F_n$ or $F_{n+k}=(1+L)^k F_n$ and $a = 1$.
  \end{sol}
  \end{problem}
  ...
  \begin{problem}
  Consider the process $(x_t)$ with integer time $t$ and $y_t=x_{-t}$. 
  
  Which reasoning is good and why?
  \begin{enumerate}
  \item $L y_t = L x_{-t} = x_{-t-1}$;
  \item $L y_t = y_{t-1} = x_{-t+1}$;
...
% before any arima related theorem


\begin{leftbar}
The process $(u_t)$ is called a white noise if $\E(u_t) = 0$, $\Var(u_t) = \sigma^2$ and $\Cov(u_t, u_s) = 0$ for $t\neq s$.

The process $(y_t)$ is called weakly stationary if $\E(y_t) = \mu$ and $\Cov(u_t, u_s) = \gamma_{t-s}$.

The process $(y_t)$ is called strictly stationary if the distribution law of $(y_t, y_{t+1}, \dots, y_{t+k})$ does not depend on $t$ for all $k$.
\end{leftbar}
...
\begin{problem}
  Recall the definition of a martingale, Markov chain and stationary process. 

  Provide an explicit example for all 8 possible combination of these properties. 
  \begin{sol}
...
\begin{problem}
  Each process can be weakly stationary or not, strictly stationary or not. 

  Provide an explicit example for all four possible combination of these properties. 
  \begin{sol}
    \begin{enumerate}
      \item $u_t \sim \cN(0;1)$ and independent;
      \item $u_t \sim \cN(0;1)$ for $t>1$, $u_1$ takes values $-1$ or $1$ with probability $1/2$.
      All $(u_t)$ are independent.
      \item Variables $u_t$ are independent and identically distributed with infinite $\E(u_t)$.
      \item $u_t \sim \cN(t; 1)$ and independent.
    \end{enumerate}
...
\begin{problem}
  Let $y_t$ be a stationary process. 
  Check whether the following processes are stationary. 
  For stationary $(z_t)$ express its autocovariance function in terms of an autocovariance function of $(y_t)$.
  \begin{enumerate}
  \item $z_t = 2y_t$;
  \item $z_t = y_t + 1$;
  \item $z_t = \Delta y_t$;
  \item $z_t = 2y_t + 3y_{t-1}$.
  \end{enumerate}
  \begin{sol}
  \begin{enumerate}
    \item stationary, $\gamma^z_k = 4\gamma^y_k$.
    \item stationary, $\gamma^z_k = \gamma^y_k$.
    \item stationary;
    \item stationary;
  \end{enumerate}
  \end{sol}
  ...
  \begin{problem}
  Let $u_t$ be a white noise. 
  Consider the equation $y_t = 2 + 0.5 y_{t-1} + u_t$ where $t \geq 0$ and each $y_t$ may depend only on $u_t$, $u_{t-1}$, \dots, $u_0$.
  
  \begin{enumerate}
  \item Find $\E(y_t)$ and $\Var(y_t)$ and check the stationarity of the process in the following cases:
  \begin{enumerate}
  \item $y_0 = 0$
  \item $y_0 = 4$
  \item $y_0 = 4 + u_0$
  \end{enumerate}
  \item Find initial condition $y_0$ such that the resulting process is stationary.
  \item In some books and internet sources it is written something like «the process $y_t = 2 + 0.5 y_{t-1} + u_t$ is stationary as $\abs{0.5} < 1$».
...
\end{enumerate}
  ...
  \begin{enumerate}
    \item
    \item Process $(y_t)$ is stationary for $y_0 = 4 +\frac{2}{\sqrt{3}}u_0$. 
    \item Expression $y_t = 2 + 0.5 y_{t-1} + u_t$ is only an equation. 
    There are infinitely many stochastic processes that satisfy this equation. 
    Most of them are non-stationary, but it has a stationary solution. 
  \end{enumerate}
  \end{sol}
  \end{problem}
  ...
  \begin{problem}
  I remove odd numbered observations from a stationary process $(x_t)$ and obtain a new process $(y_t)$.
  
  \begin{enumerate}
    \item Express the process $(y_t)$ in terms of the process $(x_t)$.
    \item Is the resulting process $(y_t)$ stationary?
    \item Find the autocovariance function of $(y_t)$ in terms of the autocovariance function of $(x_t)$.
  \end{enumerate}
  
  \begin{sol}
  \begin{enumerate}
    \item $y_t = x_{2t}$;
    \item Yes, $(y_t)$ is stationary.
    \item 
  \end{enumerate}
  \end{sol}
  \end{problem}
  ...
  \begin{problem}
  Elon Musk observes a stationary process $(x_t)$ and is very bored. 
  He throws an fair coin every time he observes a next value. 
  If the coin lands on head then he keeps the observed value as the next value of $(y_s)$, otherwise he discards the value and does not record it.
  ...
  \begin{sol}
    \begin{enumerate}
      \item The process $(y_t)$ is stationary.
      \item 
    \end{enumerate}
  \end{sol}
  \end{problem}
  ...
  \begin{problem}
  Consider the process $(u_t)$ for $t\geq 0$ where all $u_t$ are independent and uniformly distributed on $[-1;1]$.
  \begin{enumerate}
  \item Is the process $(u_t)$ a white noise?
  \end{enumerate}

  Let's draw the process $(u_t)$ and consider a new process $x_t$ for $t \geq 1$.
  Define $x_t = 1$ if the segment joining $u_t$ and $u_{t-1}$ intersects the abscissa and $x_t = 0$ otherwise.  
  
  \begin{enumerate}
    \item Find $\P(x_t = 1)$, $\E(x_t)$.
    \item Find $\Cov(x_t, x_{t+1})$ and $\Cov(x_t, x_{t+2})$.
    \item Is $(x_t)$ stationary? Is it a white noise?
  \end{enumerate}

  Now let's count the cumulative number of abscissa intersetions, $s_t = x_1 + x_2 + \dots + x_t$.
  
  \begin{enumerate}[resume]
  \item Find $\E(s_t)$.
  \item What is the distribution of $s_t$?
  \item Find approximately $\P(s_{100} > 55)$.
  \end{enumerate}
  
  \begin{sol}
  \begin{enumerate}
    \item $\P(x_t = 1) = \E(x_t) = 1/2$;
    \item $\Cov(x_t, x_s) = 0$ for $t\neq s$;
    \item The process $(x_t)$ is stationary but not a white noise as $\E(x_t) \neq 0$.
    \item $\E(s_t) = t/2$;
    \item $s_t \sim \dBin(t, 1/2)$;
  ...
  \begin{problem}
  The variables $x_t$ are independent and take values $0$ or $1$ with equal probabilities.
  The variables $y_t$ are independent and normal $\cN(0;24)$.
  The processes $(x_t)$ and $(y_t)$ are independent.
  
  Consider two processes:
  \[
  a_t = x_t (1-x_{t-1}) y_t \quad \text{and} \quad b_t = y_{t-1} y_t.
  \]
    \begin{enumerate}
    \item Are $(a_t)$ identically distributed? And $(b_t)$?
    \item Are $(a_t)$ independent? And $(b_t)$?
    \item Is $(a_t)$ a white noise? And $(b_t)$?
  \end{enumerate}
  \begin{sol}
  
  \begin{enumerate}
    \item $(a_t)$ are identically distributed; $(b_t)$ are identically distributed.
    \item $(a_t)$ are dependent, for example if $a_t \neq 0$ then $a_{t-1} = a_{t+1} = 0$; $(b_t)$ are dependent.
    \item $(a_t)$ is a white noise; $(b_t)$ is a white noise.
  \end{enumerate}
  ...
  \begin{problem}
  The variables $(u_t)$ are independent and identically continuously distributed.
  
  Let's call the time moment $t$ a turning point («поворотная точка») if it is the local extremum, i.e. it is higher than both of his neighbors or is lower than both of his neighbors. 
  
  Consider the process $(z_t)$ where $z_t = 1$ if $t$ is a turning point and $z_t = 0$ if not. 
  
  
  \begin{enumerate}
    \item Find $\P(z_t = 1)$ and $\E(z_t)$.
    \item Find $\Cov(z_1, z_2)$, $\Cov(z_1, z_3)$, $\Cov(z_1, z_4)$.
    \item Is $(z_t)$ a stationary process?
    \item Represent $z_t$ in a form $z_t = \mu + v_t + \alpha_1 v_{t-1} + \alpha_2 v_{t-2}$, where $v_t$ is a white noise. 
    \item Are $(v_t)$ in point (d) independent?
  \end{enumerate}    
  Now let's also count the number of observed turning points by $s_t = z_2 + \ldots + z_{t-1}$.
  The values $z_1$ and $z_t$ do not enter the sum, as we do not count boundary points as turning points. 
\begin{enumerate}
  \item Find $\E(s_t)$ and $\Var(s_t)$.
  \item Is $(s_t)$ stationary?
  \end{enumerate}

  \begin{sol}
    \begin{enumerate}
    \item $\P(z_t = 1) = \E(z_t) = 2/3$;
    \item $\Cov(z_1, z_2) = $, $\Cov(z_1, z_3) = $, $\Cov(z_1, z_4) = 0$;
    \item $(z_t)$ is stationary;
    \item 
    \item 
    \item $\E(s_t) = (t-2) \cdot 2/3$, $\Var(s_t) = (16t - 29)/90$;
    \item $(s_t)$ is not stationary;
    \end{enumerate}
  ...
  \begin{problem}
    The processes $(a_t)$ and $(b_t)$ are stationary. 
    The correlation $\Corr(a_t, b_t) = 0$ for all $t$. 
    
    Consider the product $y_t = a_t b_t$ and the sum $x_t = a_t + b_t$.

  
    \begin{enumerate}
      \item Is the process $(x_t)$ stationary? Provide a proof or a counter-example.
      \item Is the process $(y_t)$ stationary? Provide a proof or a counter-example.
      \item How the answers to (a) and (b) will change if  $\Corr(a_t, b_s) = 0$ arbitrary $t$ and $s$.
    \end{enumerate}
  \begin{sol}
  \begin{enumerate}
  \item The process $(x_t)$ may not be stationary;
  \item The process $(y_t)$ may not be stationary;
  \item The process $(x_t)$ is stationary, but $(y_t)$ may not. 
  \end{enumerate}
  \end{sol}
  \end{problem}
...
% the concept of partial correlation outside ARMA process

\begin{problem}
  We throw a fair dice three time, let's denote the results by $X_1$, $X_2$ and $X_3$. 
  Consider the sums $L=X_1+X_2$, $R=X_2+X_3$ and $S=X_1+X_2+X_3$.
\begin{enumerate}
\item Intuitively without calculations find the sign of ordinary and partial correlations $\Corr(L, R)$, $\Corr(L, S)$, $\pCorr(L, R; S)$,
  $\pCorr(L, S; R)$, $\Corr(X_1, R)$, $\pCorr(X_1, R; S)$, $\pCorr(X_1, R; L)$, $\pCorr(L, R; X_2)$, $\pCorr(L, R; X_1)$;
\item Try to find the exact value of all ordinary and partial calculations intuitively without calculations.
\item Calculate all the values honestly.
\end{enumerate}
\begin{sol}
...
\begin{problem}
  The random variable $Z$ takes values $0$ and $1$ with equal probabilities.
  We know the conditional distribution of the vector $X = (X_1, X_2)$ given $Z$:
  \[
    \begin{pmatrix}
      X_1 \\
      X_2 \\
    \end{pmatrix} \mid Z=0 \sim \cN\left(
      \begin{pmatrix}
        0 \\
        0 \\
      \end{pmatrix};
  ...
  \[
    \begin{pmatrix}
      X_1 \\
      X_2 \\
    \end{pmatrix} \mid Z=1 \sim \cN\left(
      \begin{pmatrix}
        1 \\
        1 \\
      \end{pmatrix};
  ...
  \begin{enumerate}
    \item Find the partial correlation $\pCorr(X_1, X_2; Z)$.
    \item Find the conditional correlation $\Corr(X_1, X_2 \mid Z)$.
  \end{enumerate}
  
    \begin{sol}
  Projections: $\tilde X_1 = X_1 + Z$; $\tilde X_2 = X_2 + Z$; $\E(X_i|Z)=1-Z$; $\Cov(X_i, Z)=-1/4$;
  
  Variable $Z$ has Bernoulli distribution, hence  $\E(Z)=1/2$ и $\Var(Z)=1/4$;
...
\begin{problem}
The random vector $X = (X_1, X_2, X_3)$ has multivariate normal distribution:
\[
  \begin{pmatrix}
    X_1 \\
    X_2 \\
...
\]
\begin{enumerate}
  \item Find the partial correlation $\pCorr(X_1, X_2; X_3)$.
  \item Find the conditional correlation $\Corr(X_1, X_2 \mid X_3)$.
\end{enumerate}
...
\begin{problem}
  ...
  \begin{sol}
    \begin{enumerate}
      \item Infinitely many non-stationary solutions.
      \item $\lambda^2 - \lambda - 1 = 0$, $\lambda = (1 \pm \sqrt{5}) / 2$; there is exactly one stationary solution, this solution can not be represented as $MA(\infty)$ with respect to $(u_t)$.
      \item No; 
    \end{enumerate}
  \end{sol}
\end{problem}
...
\begin{problem}
The process $(u_t)$ is a white noise, $t \in \ZZ$.
Which of the following equations admit at least one stationary solution?
\begin{enumerate}
\item $y_t=1+u_t+0.5u_{t-1}+0.25u_{t-2}$;
\item $y_t=-2y_{t-1}-3y_{t-2}+u_t+u_{t-1}$;
\item $y_t=-0.5y_{t-1} + u_t$;
\item $y_t=1-1.5 y_{t-1} - 0.5 y_{t-2} + u_t - 1.5u_{t-1} - 0.5u_{t-2}$;
\item $y_t=1+0.64y_{t-2}+u_t+0.64u_{t-1}$;
\item $y_t=1+t+u_t$;
\item $y_t=1+y_{t-1}+u_t$.
\end{enumerate}
\begin{sol}

\begin{enumerate}
\item $y_t = 1+u_t+0.5u_{t-1}+0.25u_{t-2}$ admits a stationary solution.
\item $y_t = -2y_{t-1}-3y_{t-2}+u_t+u_{t-1}$
\item $y_t = -0.5y_{t-1} + u_t$ admits a stationary solution.
\item $y_t = 1-1.5 y_{t-1} - 0.5 y_{t-2} + u_t - 1.5u_{t-1} - 0.5u_{t-2}$
\item $y_t = 1+0.64y_{t-2}+u_t+0.64u_{t-1}$ admits a stationary solution.
\item $y_t = 1 + t + u_t$ does not admit a stationary solution.
\item $y_t = 1 + y_{t-1} + u_t$ does not admit a stationary solution.
\end{enumerate}
\end{sol}
\end{problem}

\begin{problem}
  The process $y_t$ is stationary and is generated by equation $y_t = 1 + 0.5 y_{t-1} + u_t$ where $(u_t)$ is a white noise. 
  
  We have $10000$ observations.
  
  \begin{enumerate}
    \item What will be the approximate OLS estimates in the regression $\hat y_t = \hat\beta_1 + \hat\beta_2 y_{t-1}$?
    \item What will be the approximate OLS estimates in the regression $\hat y_t = \hat\gamma_1 + \hat\gamma_2 y_{t+1}$?
  \end{enumerate}
  
  \begin{sol}
  $\hat\beta_1 \approx \hat\gamma_1 \approx 1$ and $\hat\beta_2 \approx \hat\gamma_2 \approx 0.5$ as 
  estimates can be recovered from sample autocovariance function and sample mean. 
  \end{sol}
  \end{problem}
...
\begin{problem}
  ...
  Consider the recurrence equation $y_t = 2 + 0.5 y_{t-1} + u_t$ for $t \in \{0, 1, 2, \dots \}$ and independent $u_t \sim \cN(0;\sigma^2)$.

  \begin{enumerate}
    \item Find three different stationary processes $(y_t)_{t=0}^{\infty}$ that satisfy this equation. 
    \item Will the forecasts for these three solutions from (a) be different?
    \item How the answer to (a) will change if $t \in \ZZ$ and we need to find $(y_t)_{t=-\infty}^{\infty}$?
  \end{enumerate}
  \begin{sol}
    \begin{enumerate}
      \item $y_0 = 4 + 2u_0/\sqrt{3}$, $y_0 = 4 - 2u_0/\sqrt{3}$,  \dots
      \item The forecasts are the same as the distribution of the vector $y = (y_1, y_2, \dots, y_n)$ is the same.
      \item There will be almost surely unique stationary solution. 
    \end{enumerate}
  \end{sol}
\end{problem}

\begin{problem}
This exercise provides an elementary proof that a unit root corresponds to the absence of stationary solutions.  

Let $(y_t)$ be a stationary process with autocovariance function $\gamma_k$ and $(u_t)$ — a white noise with variance $\sigma^2$.
\begin{enumerate}
  \item Using trivial bounds for correlation deduce the possible range for $\Cov(y_{0}, u_{1} + u_{2} + \dots + u_{k})$.
  \item Prove that $(y_t)$ can't satisfy an equation $y_t = y_{t - 1} + u_t$.
  \item Prove that $(y_t)$ can't satisfy an equation $(1 - L) (1 - 0.5L) y_t = u_t$.
\end{enumerate}
Let $(m_t)$ be an $MA(2)$ process with the autocovariance function $c_k$.
\begin{enumerate}[resume]
  \item Find $\lim_{k \to \infty} \Var(m_1 + m_{2} + \dots + m_{k}) / k$.
  \item Prove that $(y_t)$ can't satisfy an equation $(1 - L) (1 - 0.5L) y_t = m_t$ if $c_0 + c_1 + c_2 \neq 0$.
  \item Provide an example of $MA(2)$ process with autocovariance function $c_k$ that satisfies the equation $c_0 + c_1 + c_2 = 0$.
  \item Prove the general theorem: 
  the recurrence equation $a(L) y_t = b(L) u_t$ where $(u_t)$ is a white noise, $a(1) = 0$ and $b(1) \neq 0$ can't have a stationary solution $(y_t)$.
\end{enumerate}


\begin{sol}
\begin{enumerate}
  \item $\abs{\Cov(y_0, u_1 + u_2 + \dots + u_k)} \leq \sqrt{k \gamma_0 \sigma^2}$.
  \item From the recurrence equation it follows that 
  \[
  \Var(y_{t + k}) = \Var(u_1 + u_2 + \dots  + u_k + y_0).
  \]
  Hence, 
  \[
  0 = k \sigma^2 + 2 \Cov(y_0, u_1 + u_2 + \dots + u_k).
  \]
  That is not possible as the covariance is bounded by something proportional to $\sqrt{k}$.
  \item Let's denote $(1 - 0.5L)y_t$ by $x_t$. 
  The process $(x_t)$ is also stationary and can't satisfy equation $x_t = x_{t- 1} + u_t$.
  \item 
  \[
    \Var(m_1 + m_2 + \dots + m_k) = k c_0 + (k - 1) c_1 + (k - 2) c_2
  \]
  Hence the limit is equal to the sum of covariances $c_0 + c_1 + c_2$.
  \item Now the process $x_t = (1 - 0.5L) y_t$ should satisfy $x_t = x_{t- 1} + m_t$.
  By the same reasoning
  \[
  0 = k c_0 + (k - 1) c_1 + (k - 2) c_2 + 2 \Cov(x_0, u_1 + u_2 + \dots  + u_k)
  \]
  If $c_0 + c_1 + c_2 \neq 0$ then covariance can't compensate the linear growth of the first three terms. 
  \item Any process with unit root in $MA$-polynomial will do. 
  For example, $m_t = (1 - L) (1 - 0.5L) u_t = u_t - 1.5 u_{t-1} + 0.5 u_{t-2}$.

  \item The same logic applies :)
...
\begin{problem}
  Is it possible that a stationary $(y_t)$ satisfies the equation $y_t = 5 + 2y_{t-1} - y_{t-2} + u_t + u_{t-2}$ and $(u_t)$ is a white noise process?
  Provide a proof or a counter-example.
  \begin{sol}
If $\E(y_t) = \mu$ then $\mu = 5 + 2\mu - \mu + 0$ and we arrive to a contradiction.
  \end{sol}
\end{problem}
...
\begin{problem}
  Is it possible that $\Cov(y_t, u_s) = 0$ for all $t < s$, $(y_t)$ satisfies the equation $y_t = 5 + 2y_{t-1} + u_t$ and $(u_t)$ is a white noise process?
  Provide a proof or a counter-example.
  \begin{sol}
Observe that $\Var(y_t) = 4 \Var(y_{t-1}) + \sigma^2$ and we arrive to a contradiction.
  \end{sol}
\end{problem}
...
\]
\begin{sol}
The right answer is $MA(0)$ process or white noise. 
Explanation: if one adds $AR(p)$ process to an independent white noise then one obtains $ARMA(p, p)$ process. 
Let's denote the processes by $a_t$, $b_t$ and $a_t + b_t = c_t$.
We know that $A(L)a_t = u_t$, where $A(L)$ is some polynomial of order $p$.
Hence $A(L)a_t + A(L)b_t = A(L)c_t$.
All autocorrelations of $A(L)a_t$ are zeros. 
The process $b_t$ is a white noise, hence all autocorrelations of $A(L)b_t$ are zeros for order $p + 1$, $p + 2$, etc.
One may deduce that $A(L)c_t$ can be written as $MA(p)$ process and $c_t \sim ARMA(p, p)$.
\end{sol}
\end{problem}
...
\begin{problem}
  ...
  \end{cases}    
  \]
  Let $s_{100} = 3$, $s_{99} = -2$, $\ell_{100} = 100$.
  \begin{enumerate}
    \item Find 95\% predictive interval for $y_{102}$.
    \item Find 95\% predictive interval for $s_{121}$.
    \item Write this model in the form $A(L) y_t = B(L) u_t$, where $A(L)$ and $B(L)$ are lag polynomials.
    \item Classify this model as $SARIMA(p, d, q)(p^s, d^s, q^s)$: explicitely state $p$, $d$, $q$, $p^s$, $d^s$, $q^s$.
  \end{enumerate}
  
  \begin{sol}

    Observe that $(1 - L) \ell_t = 0.3 u_t$ and $(1 - L^2) s_t = 0.1 u_t$. 
    Hence it is sufficient to multiply $y_t$ by $(1 - L^2)$ to get rid of $\ell_t$ and $s_{t_2}$ in the right hand side. 
  \end{sol}
\end{problem}
...
\begin{problem}
  ...
  \[ 
      \begin{cases}
          y_t = \sigma_t \nu_t \\
          \sigma_t^{2}= 1 + 0.2 y_{t-1}^{2}+ 0.3 \sigma_{t-1}^{2}    \\
          \nu_t \sim \cN(0;1)
      \end{cases}     
  \]
