...
\begin{enumerate}
  \item Using trivial bounds for correlation deduce the possible range for $\Cov(y_{t-k}, u_{t} + u_{t-1} + \dots + u_{t + 1 - k})$.
...
\begin{enumerate}[resume]
  \item Find $\lim_{k \to \infty} \Var(m_t + m_{t-1} + \dots + m_{t+1 - k}) / k$.
...
\begin{sol}
\begin{enumerate}
  \item $\abs{\Cov(y_{t-k}, u_{t} + u_{t-1} + \dots + u_{t + 1 - k})} \leq \sqrt{k \gamma_0 \sigma^2}$.
  \item From the recurrence equation it follows that 
  \[
  \Var(y_t) = \Var(u_t + u_{t-1} + \dots  + u_{t+1 - k} + y_{t - k}).
  \]
  Hence, 
  \[
  0 = k \sigma^2 + 2 \Cov(y_{t-k}, u_{t} + u_{t-1} + \dots + u_{t + 1 - k}).
  \]
  ...
  \item 
  \[
    \Var(m_t + m_{t-1} + \dots + m_{t+1 - k}) = k c_0 + (k - 1) c_1 + (k - 2) c_2
  \]
  ...
  \[
  0 = k c_0 + (k - 1) c_1 + (k - 2) c_2 + 2 \Cov(x_{t-k}, u_t + u_{t-1} + \dots  + u_{t+1 - k})
  \]
