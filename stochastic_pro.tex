...
\section{Conditional expected wrt to random variables or sigma-algebras}
...
\begin{problem}
  ...
  Find $\E(Y \mid \cF)$, $\Var(Y \mid \cF)$, $\P(Y = 3 \mid \cF)$.
  \begin{sol}
  \end{sol}
\end{problem}
...
\begin{problem}
  \begin{problem}
    ...
    \begin{enumerate}
    \item Find $\E(R+L \mid M)$, $\E(M \mid R+L)$.
    ...
    \begin{sol}
    $\E(R+L \mid M)=n-M$, $\E(M \mid R+L)=n-R-L$, $\E(R \mid L)=(n-L)\frac{r}{1-l}$, $\Var(R \mid L)=(n-L)\frac{r}{1-l}\frac{1-r-l}{1-l}$, $\P(\E(R \mid L)=0)=l^{n}$, $\P(R=0 \mid  L)=\left(\frac{r}{r+m}\right)^{n-L}$
    \end{sol}
    \end{problem}
